...
* xref:deployment:core/install.adoc#install-core-instance[OpenNMS package repositories]
* JICMP and JICMP6 packages to allow sending ICMP messages with Java
* RRDtool from the official package repositories and JRRD2 to use RRDtool as it is now the default
* The Java-based RRD implementation JRobin has been removed and is no longer available.
...
* xref:deployment:core/install.adoc#setup-postgresql[PostgreSQL server] (installed and initialized)
...
== (Automated) Compile and assemble from source
A script is provided to help with building and bringing up OpenNMS on both Mac and Linux (Ubuntu).

=== Check for dependencies
You can check required dependencies by running 
...
=== Build and run OpenNMS
You can build and run OpenNMS by running the following command.
...
Follow xref:deployment:core/install.adoc#set-up-core-instance[Set up the core instance] from the deployment guide; the procedure is the same.

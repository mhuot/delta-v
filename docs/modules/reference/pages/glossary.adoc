...
This RRD implementation was available in previous releases of {page-component-title}. It was deprecated in version 34 and removed in version 35.
...
You can use it as an alternative persistence strategy (instead or RRDtool).
...
PostgreSQL scales up, but not out (see xref:deployment:core/install.adoc#setup-postgresql[Set up PostgreSQL]).

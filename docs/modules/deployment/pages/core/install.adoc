...
DNS lookups may be performed by different processes, including while provisioning new nodes and when receiving syslog messages.
...
One way to mitigate DNS performance is to set up your {page-component-title} as a secondary DNS server so that it is able to self-host DNS records for your network.

ifeval::["{page-component-title}" == "Horizon"]
== Quick installation

If you want to get started quickly with OpenNMS Horizon on a bare metal system or a virtual machine, you can check out our https://github.com/opennms-forge/opennms-install[quick installation scripts].
...
* Script is only for new installations and does not deal with existing installations or upgrades.

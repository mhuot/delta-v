...
compile: maven-structure-graph
	$(MAVEN_BIN) install $(MAVEN_ARGS) -DskipTests=true -Dbuild.profile=default -Droot.dir=$(WORKING_DIRECTORY) -Dbuild.skip.tarball=false -Prun-expensive-tasks -Psmoke -Dbuild.type=production -Dbuild.sbom=true -pl '!core/db-init' 2>&1 | tee $(ARTIFACTS_DIR)/mvn.compile.log
...
quick-compile: maven-structure-graph
	$(MAVEN_BIN) install $(MAVEN_ARGS) -T 1C -DskipTests=true -Dbuild.profile=default -Droot.dir=$(WORKING_DIRECTORY) -Dcyclonedx.skip=true -pl '!core/db-init' 2>&1 | tee $(ARTIFACTS_DIR)/mvn.quick-compile.log
